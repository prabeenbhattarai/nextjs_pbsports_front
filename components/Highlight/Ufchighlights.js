...
const UfcHighlights = ({ ufchighlight }) => {
  const scrollRef = useRef(null);

  return (
    <Center>
      {ufchighlight.length > 0 && (
        <>
          <SectionTitle hide={ufchighlight.length === 0}>UFC Replay</SectionTitle>

          <ScheduleGridContainer>
            {ufchighlight.length > 6 && (
              <LeftScrollButton
                hide={scrollRef.current?.scrollLeft === 0}
            ...
            <ScheduleGrid ref={scrollRef}>
              {ufchighlight.map((highlightItem) => (
                <HighlightBox key={highlightItem._id} {...highlightItem} />
              ))}
            </ScheduleGrid>
            {ufchighlight.length > 6 && (
              <RightScrollButton
                hide={scrollRef.current?.scrollLeft === (scrollRef.current?.scrollWidth - scrollRef.current?.clientWidth)}
                onClick={() => scrollRef.current.scrollBy({ left: 300, behavior: 'smooth' })}
              >
