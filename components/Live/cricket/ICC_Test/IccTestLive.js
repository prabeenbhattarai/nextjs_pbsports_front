...
import IccTestBox from "./IccTestbox";
...
const CopaLive = ({ icctest }) => {
  ...
  return (
    <Center>
      <>
        <SectionTitle hide={liveItems.length === 0}>ICC World Test Championship Alert</SectionTitle>
        ...
        <SectionTitle hide={recentlyScheduledItems.length === 0}>ICC World Test Championship</SectionTitle>
        <ScheduleGridContainer>
          ...
          <ScheduleGrid ref={recentlyScheduledScrollRef}>
            {recentlyScheduledItems.map((item) => (
              <IccTestBox key={item._id} {...item} showScheduledTimeTag={true} />
            ))}
          </ScheduleGrid>
        </ScheduleGridContainer>
      </>
