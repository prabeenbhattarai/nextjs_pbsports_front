...
import IccT20Box from "./IccT20Box";
...
const iCCT20Live = ({ icct20 }) => {
  ...
  return (
    <Center>
      <>
        <SectionTitle hide={liveItems.length === 0}>ICC T20 Worldcup Alert</SectionTitle>
        ...
        <SectionTitle hide={recentlyScheduledItems.length === 0}>ICC T20 Worldcup Events</SectionTitle>
        <ScheduleGridContainer>
          ...
          <ScheduleGrid ref={recentlyScheduledScrollRef}>
            {recentlyScheduledItems.map((item) => (
              <IccT20Box key={item._id} {...item} showScheduledTimeTag={true} />
            ))}
          </ScheduleGrid>
        </ScheduleGridContainer>
      </>
