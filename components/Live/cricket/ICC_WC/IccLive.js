...
const IccLive = ({ iccwc }) => {
  ...
  return (
    <Center>
      <>
        <SectionTitle hide={liveItems.length === 0}>ICC Cricket Worldcup Alert</SectionTitle>
        ...
        <SectionTitle hide={recentlyScheduledItems.length === 0}>ICC Cricket Worldcup Events</SectionTitle>
        <ScheduleGridContainer>
          ...
          <ScheduleGrid ref={recentlyScheduledScrollRef}>
            {recentlyScheduledItems.map((item) => (
              <IccBox key={item._id} {...item} showScheduledTimeTag={true} />
            ))}
          </ScheduleGrid>
        </ScheduleGridContainer>
      </>
