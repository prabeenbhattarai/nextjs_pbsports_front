...
const AsiaLive = ({ asiacup }) => {
  ...
  return (
    <Center>
      <>
        <SectionTitle hide={liveItems.length === 0}>Asia Cup Alert</SectionTitle>
        ...
        <SectionTitle hide={recentlyScheduledItems.length === 0}>Asia Cup Events</SectionTitle>
        <ScheduleGridContainer>
          ...
          <ScheduleGrid ref={recentlyScheduledScrollRef}>
            {recentlyScheduledItems.map((item) => (
              <AsiaBox key={item._id} {...item} showScheduledTimeTag={true} />
            ))}
          </ScheduleGrid>
        </ScheduleGridContainer>
      </>
