...
const EuroLive = ({ euro }) => {
  ...
  return (
    <Center>
      <>
        <SectionTitle hide={liveItems.length === 0}>Euro Cup Alert</SectionTitle>
        ...
        <SectionTitle hide={recentlyScheduledItems.length === 0}>Euro Cup Events</SectionTitle>
        <ScheduleGridContainer>
          ...
          <ScheduleGrid ref={recentlyScheduledScrollRef}>
            {recentlyScheduledItems.map((item) => (
              <EuroBox key={item._id} {...item} showScheduledTimeTag={true} />
            ))}
          </ScheduleGrid>
        </ScheduleGridContainer>
      </>
