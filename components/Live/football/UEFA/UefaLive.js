...
const UefaLive = ({ uefa }) => {
  ...
  return (
    <Center>
      <>
        <SectionTitle hide={liveItems.length === 0}>UEFA Champions Alert</SectionTitle>
        ...
        <SectionTitle hide={recentlyScheduledItems.length === 0}>UEFA Champions Events</SectionTitle>
        <ScheduleGridContainer>
          ...
          <ScheduleGrid ref={recentlyScheduledScrollRef}>
            {recentlyScheduledItems.map((item) => (
              <UefaBox key={item._id} {...item} showScheduledTimeTag={true} />
            ))}
          </ScheduleGrid>
        </ScheduleGridContainer>
      </>
