...
const FifaLive = ({ fifa }) => {
  ...
  return (
    <Center>
      <>
        <SectionTitle hide={liveItems.length === 0}>FIFA WorldCup Alert</SectionTitle>
        ...
        <SectionTitle hide={recentlyScheduledItems.length === 0}>FIFA WorldCup Events</SectionTitle>
        <ScheduleGridContainer>
          ...
          <ScheduleGrid ref={recentlyScheduledScrollRef}>
            {recentlyScheduledItems.map((item) => (
              <FifaBox key={item._id} {...item} showScheduledTimeTag={true} />
            ))}
          </ScheduleGrid>
        </ScheduleGridContainer>
      </>
